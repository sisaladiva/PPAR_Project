...
int golden_claw_search_mpi(int maxres, u64 k1[], u64 k2[], int rank, int num_processes) {
    ...
    if (local_dict_size < num_processes * 2) {
	...
	printf("A LA BASE dictionary size: %sB\n", hdsize);
            ...
            for (u64 z = 0; z < N; z += 4) {
                ...
                for (int i = 0; i < 4; i++) {
                    if ((y[i] & 1) == parity) { // Remplacer modulo par AND pour optimiser
                        ...
                        if (nx > 256) {
                            fprintf(stderr, "dict_probe result exceeds buffer size\n");
                            exit(EXIT_FAILURE);
                        }
                        ...
                        for (int j = 0; j < nx; j++) {
                            if (is_good_pair(local_x[j], z + i)) {
                                if (local_nres >= 1024) {
                                    fprintf(stderr, "Local results array out of bounds\n");
                                    exit(EXIT_FAILURE);
                                }
