    ...
    for (int parity = 0; parity < 2; parity++) {
        ...
        #pragma omp parallel reduction(+:ncandidates)
        {
            ...
            for (u64 z = 0; z < N; z+=4) {
                ...
                // SIMD vectorized computation for g(z) over 4 elements
                #pragma omp simd
                for (int i = 0; i < 4; i++) {
                    if ((z + i) < N) {          // Process only valid indices
                        y[i] = g(z + i);
                    } else {
                        printf("Invalid index: %llu\n", z + i);
                        y[i] = 0;               // Assign a default value to avoid uninitialized access
                    }
                }

                // Batch processing with reduced branching
                #pragma omp simd
                for (int i = 0; i < 4; i++) {
                    if ((z + i) >= N) continue; // Skip invalid indices
                    if ((y[i] & 1) == (u64) parity) {
